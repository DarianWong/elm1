...
# 只考虑前十个被助力账号
max_assist_accounts = min(10, len(zlck_list))  # 确保不超过账户总数
dzl_num = 0

for zlck in zlck_list[:max_assist_accounts]:  # 只遍历前十个被助力账号
    dzl_num += 1
    lyb = LYB(zlck)
    actid, shareId = lyb.yqm()

    if actid is None or shareId is None:
        print(f"❎获取助力id失败，跳过被助力账号{dzl_num}")
        continue  # 跳过当前 zlck，继续下一个
    ...
    for i, ck in enumerate(cookies):
        ...
        if zlcs < 3:  # 确保助力次数小于3
            print(f"======开始为被助力账号{dzl_num} 助力，第{i + 1}/{len(cookies)} 个助力账号======")
            ...
            elif a is False:  # 助力失败
                data[f"{usid}"] = 3  # 将助力次数设置为3，防止再次助力
                with open(filename, 'w') as file:
                    json.dump(data, file, indent=4)
                print("2s后进行下一个账号")
